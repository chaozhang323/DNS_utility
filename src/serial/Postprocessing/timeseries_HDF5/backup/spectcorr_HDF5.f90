...
program spectcorr
  ...
  integer :: icalPSD, icalcorrxt, icalcorrxy, icalphasespeed, icalWNSD_ky, icalWNSD_kx, icalky_freq, icalkx_freq, ical3D, ical_skewness
  ...
  if(ireadTSVol.eq.1) then  ! Using 3D timeseries volume dataset
    ...
    ! Calculating kx-freq spectrum
    if(icalkx_freq.eq.1) then
      call InitSpect2d_ty(ntpoint_total, nxpoint_Vol, nsection, iwindow, ioverlap, dt_sample, dx_Vol, ntrans, nperiod)
      allocate(tsdata2d(1:ntpoint_total, 1:nxpoint_Vol),tsdata2d_tmp(1:ntpoint_total, 1:nxpoint_Vol))
      tsdata2d = 0.d0

      allocate(spect2dtmp(ntrans,nxpoint_Vol), spect2dave(ntrans,nxpoint_Vol))
      do m = 1, nvar_output
        spect2dave = 0.d0; numpt1 = 0
        tmean = 0.d0; tmeanenergy = 0.d0
        fileoutput = 'kx-freq_powerspect_k'//fnum3//'_'//fnum4//'_i'//fnum1//'-'//fnum2//'_'//trim(varout_Vol(m))//'_nsection'//fnum5
        print *, 'fileoutput =', trim(fileoutput)
        do n=1, ntpoint_total
          do i=1, nxpoint_Vol
            tsdata2d_tmp(n,i) = sum(buffer_Vol(n,:,i,:,m))/dble(nypoint_Vol*nzpoint_Vol)
          enddo
        enddo
        do k=1, nzpoint_Vol
        do j=1, nypoint_Vol
          tsdata2d(1:ntpoint_total,1:nxpoint_Vol) = buffer_Vol(1:ntpoint_total,k,1:nxpoint_Vol,j,m) - tsdata2d_tmp(1:ntpoint_total,1:nxpoint_Vol)
          spect2dtmp(:,:) = autospect2d(tsdata2d(1:ntpoint_total,1:nxpoint_Vol))
          spect2dave = spect2dave + spect2dtmp
          tmean = tmean + sum(tsdata2d(1:ntpoint_total,1:nxpoint_Vol))
          tmeanenergy = tmeanenergy + sum(tsdata2d(1:ntpoint_total,1:nxpoint_Vol)**2)
          numpt1 = numpt1 + 1
        enddo ! end i loop
        enddo ! end k loop
        tmean = tmean/dble(ntpoint_total*nxpoint_Vol*numpt1)
        tmeanenergy = tmeanenergy/dble(ntpoint_total*nxpoint_Vol*numpt1)
        tmeanenergy = tmeanenergy - tmean**2
    ...
    ! calculate kx-ky-freq spectrum
    if(ical3D.eq.1) then
      call InitSpect3d(ntpoint_total, nxpoint_Vol, nypoint_Vol, nsection, 1, iwindow, ioverlap, dt_sample, dx_Vol, dy_Vol, ntrans, ntrans_kx)
      allocate(tsdata3d(1:ntpoint_total,1:nxpoint_Vol,1:nypoint_Vol))
      tsdata3d = 0.d0

      allocate(spect3dtmp(ntrans,ntrans_kx,nypoint_Vol),spect3dave(ntrans,ntrans_kx,nypoint_Vol))
      do m=1, nvar_output
        spect3dave = 0.d0; numpt1 = 0
        tmean = 0.d0; tmeanenergy = 0.d0
        fileoutput = '_kx_ky_freq_k'//fnum3//'_'//fnum4//'_i'//fnum1//'-'//fnum2//'_'//trim(varout_Vol(m))//'_nsection'//fnum5
        print *, 'fileoutput =', trim(fileoutput)

        do k=1, nzpoint_Vol
          tsdata3d(1:ntpoint_total,1:nxpoint_Vol,1:nypoint_Vol) = buffer_Vol(1:ntpoint_total,k,1:nxpoint_Vol,1:nypoint_Vol,m)
          spect3dtmp(1:ntrans,1:ntrans_kx,1:nypoint_Vol) = autospect3d(tsdata3d(1:ntpoint_total,1:nxpoint_Vol,1:nypoint_Vol))
          spect3dave = spect3dave + spect3dtmp
          numpt1 = numpt1 + 1
        enddo
        spect3dave = spect3dave/dble(numpt1)
    ...
    ! calculate kx-kz-freq spectrum
    if(ical3D.eq.1) then
      call InitSpect3d(ntpoint_total, nzpoint_Vol, nxpoint_Vol, nsection, 1, iwindow, ioverlap, dt_sample, dz_Vol, dx_Vol, ntrans, ntrans_kx)
      allocate(tsdata3d(1:ntpoint_total,1:nzpoint_Vol,1:nxpoint_Vol))
      tsdata3d = 0.d0

      allocate(spect3dtmp(ntrans,nzpoint_Vol,ntrans_kx),spect3dave(ntrans,nzpoint_Vol,ntrans_kx))
      do m=1, nvar_output
        spect3dave = 0.d0; numpt1 = 0
        tmean = 0.d0; tmeanenergy = 0.d0
        fileoutput = '_kx_kz_freq_k'//fnum3//'_'//fnum4//'_i'//fnum1//'-'//fnum2//'_'//trim(varout_Vol(m))//'_nsection'//fnum5
        print *, 'fileoutput =', trim(fileoutput)

        do j=1, nypoint_Vol
          tsdata3d(1:ntpoint_total,1:nzpoint_Vol,1:nxpoint_Vol) = buffer_Vol(1:ntpoint_total,1:nzpoint_Vol,1:nxpoint_Vol,j,m)
          spect3dtmp(1:ntrans,1:nzpoint_Vol,1:ntrans_kx) = autospect3d(tsdata3d(1:ntpoint_total,1:nzpoint_Vol,1:nxpoint_Vol))
          spect3dave = spect3dave + spect3dtmp
          numpt1 = numpt1 + 1
        enddo
        spect3dave = spect3dave/dble(numpt1)
...
5       arr(l+1)=arr(j)
    ...
    subroutine Input()
      ...
      read(*,*)
      read(*,*)
      read(*,*) icalPSD, icalcorrxt, icalcorrxy, icalWNSD_ky, icalWNSD_kx, icalphasespeed, icalky_freq, icalkx_freq, ical3D, ical_skewness, icalkx_ky, icalWNSD_kz
      read(*,*)
