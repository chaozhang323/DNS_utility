...
c=======================================
      iu15=15
      !open(iu15,file='eddybl_type.dat',status='old',err=90)
      call namin(iu15)
c=======================================
c          Initialization
c=======================================
...
c=======================================
      ierr=0
      open(iunit5,file='table.dat',status='old',err=10)
      go to 20
 10   ierr=-1
      go to 70
c=======================================
...
c=======================================
      do 30 i=1,number
        !read(iunit5,210) as(i),ape(i),apep(i),aqw(i),aqwp(i),
...
c=======================================================================
      ...
      common/pro1/  pr1(7)
      common/readinput/  ireadfile,irestart     
      character*400 fname_outflow
c=======================================================================
      iu(1)=iu15
      do 10 i=2,niu-1
        iu(i)=i+5
 10   continue
      ...
      read(*,*) irestart
      read(*,*)
      read(*,'(a)') fname_outflow
      read(*,*)
      read(*,*)
      read(*,*) ireadfile
      if(ireadfile.eq.1) then !!!
        xc(1:nxc)=(/0.52,0.0708,0.09,0.5,0.6,0.89/)
        xcm(1:nxcm) = (/0.7751196,0.2009569,0.5014354,
     *                     1.8,1.0,0.0/)
        read(*,*)
        read(*,*) iu(8)
        read(*,*)
        read(*,*) nid(1), nid(2)
        read(*,*)
        read(*,*) xd(1), xd(3), xd(4),  xd(6), xd(7), xd(8),
     *            xc(6), xcm(5)
        read(*,*)
        read(*,*) xd(11),  xd(14), xd(15), xd(13), xd(16), xd(19),
     *            xd(20), pr1(1), pr1(5)
        read(*,*)
        read(*,*) xd(9), n(1), n(2), n(5), n(6), n(7), n(8), n(9),
     *            n(10), n(11)
        read(*,*)
        read(*,*) pr1(2), pr1(3), pr1(4), pr1(6), xd(5), xd(12), xd(18)
        read(*,*)
        read(*,*) pr1(7), xd(10), xd(2), n(4), xd(17), n(3)
        ...
        case(0)
          if(nid(2).eq.1) then
            xke(1:nke) = (/0.3,1.0,1.6,0.0168,1.0,0.125/)
          else
            print *, 'unknown NVISC viscous options ... '
            stop
          endif
        case(1)
          if(nid(2).eq.1) then
        ...
        case(2)
          if(nid(2).eq.0) then
            xke(1:nke) = (/1.55,2.0,0.09,1.3,1.0,0.5/)
          else
            print *, 'unknown NVISC viscous options ... '
            stop
          endif
        case(4)
          xke(1:nke) = (/0.1355,0.622,7.1,0.6666667,0.3,2.0/)
        case(6)
          xke(1:nke) = (/0.3,1.0,1.6,0.0168,1.0,26./)
        case default
            print *, 'unknown MODEL viscous options ... '
            print *, 'Not implement ... '
            stop
        end select
     ...
     *'RETHET    =',
     *'SI        ='/)
        print *, 'reading the input parameters from
     * file: eddybl_start.inp ... '
        print *, 'generating the input file (eddybl.dat)
     * for Program EDDYBL ... '
        open(iu15+200,file='eddybl.dat',status='unknown')
          a = 'irestart  ='
          write(iu15+200,100) a,irestart
          write(iu15+200,'(2a)') trim(fname_outflow)
          write(iu15+200,100) b(1), iu(8)
          do 120 i=1,nnid
            write(iu15+200,100) b(1+i),nid(i)
 120      continue
          do 130 i=1,nxc
            write(iu15+200,200) b(3+i),xc(i)
 130      continue
          do 140 i=1,nke
            write(iu15+200,200) b(9+i),xke(i)
 140      continue
          do 150 i=1,nxcm
            write(iu15+200,200) b(15+i),xcm(i)
 150      continue
          do 160 i=1,nxd
            write(iu15+200,200) b(21+i),xd(i)
 160      continue
          do 170 i=1,nn
            write(iu15+200,100) b(41+i),n(i)
 170      continue

          proval = 1.e10
          do 180 i=1,ipro
            write(iu15+200,200) b(52+i),proval(i)
 180      continue
          do 190 i=1,7
            write(iu15+200,200) b(62+i),pr1(i)
 190      continue

        !close(iu15+200)
      else
      open(iu15,file='eddybl.dat',status='old')
...
c==============================================
        if(idisk.ne.0) then
          open(iunit2,file='Check_eddybl_start.prt',status='unknown')
          write(iunit2,110)
...
c=======================================
        open(iunit5,file='Profile_inflow.dat',status='unknown')
        write(iunit5,*) mstart,ibound,iedge
        do 80 n=1,iedge
          if((model.lt.0.or.ibound.eq.0).and.ibody.eq.1) then
            !write(iunit5,200) dy(n),et(n),et(n),f(n),f(n),t(n),t(n),
 ...
 80     continue
        !write(iunit5,210) dhelp,done,dtwo,d1help,eten,eteo,itstop,
     ...
     *                ustrtm,wten,wteo,xistrt
        ...
        open(iunit5+200,file='Check_Profile_inflow.dat',
     *status='unknown')
        write(iunit5+200,'(2a)') 'variables=y,et,f,t,v,w,w,rato,ww,
     *                           uu,uv,vv,deluv'
        do 85 n=1,iedge
          if((model.lt.0.or.ibound.eq.0).and.ibody.eq.1) then
            write(iunit5+200,200) y(n),et(n),f(n),t(n),
     *                    v(n),0.,w(n),rato(n),
     *                    ww(n),uu(n),uv(n),
     *                    vv(n),deluv(n)
          else
 ...
 85     continue
    !    write(iunit5+200,210) dhelp,done,dtwo,d1help,eten,eteo,itstop,
        !write(iunit5,*) dhelp,done,dtwo,d1help,eten,eteo,itstop,
    ! *                restrt,rstrt,rstrtm,si,si1,testrt,ustrt,
    ! *                ustrtm,wten,wteo,xistrt
        endfile iunit5+200
...
!        write(iunit5) mstart,ibound,iedge
!
!        print *, 'writting file: input.dat'
!        print *, 'mstart = ', mstart, 'ibound = ', ibound, 'iedge = ', iedge
!
!        do 80 n=1,iedge
!          if((model.lt.0.or.ibound.eq.0).and.ibody.eq.1) then
!            write(iunit5) dy(n),et(n),et(n),f(n),f(n),t(n),t(n),
...
c=======================================================================
 ...
 200  format(1x,1p22e15.6)
 210  format(1x,1p6e15.6,i4,1p11e15.6)
 220  format(i4,i4,i4)
c=======================================================================
      end
...
c=======================================
      ...
      if(ireadfile.eq.1) then
        ...
        read(*,*)
        read(*,*) ireadWallProp
        if(ireadWallProp.eq.0) then
          read(*,*)
          read(*,*) (s(i),peb(i),twb(i),i=1,number)
          qw = 0.
          rvwald = 0.
          rmi = 3.048000e-01
          z(1) = s(1)
          z(number) = s(number)
          zcurv = 0.
          pebp = 0.; twbp=0.;qwp=0.;rwp(1)=0.;rp(1)=0.;
          zp=1.; zcurvp=0.
        else
          print *, 'reading file: Edge_WallProperty.dat'
          open(200,file='Edge_WallProperty.dat',status='old')
            read(200,*)
            do 10 i=1, number
            read(200,*) s(i),peb(i),twb(i),qw(i),rvwald(i),rmi(i),
     *                    z(i),zcurv(i)
 10         continue
          close(200)
          print *, 'reading file: InitialFinalSlop.dat'
          open(200,file='InitialFinalSlop.dat',status='old')
            read(200,*)
            read(200,*) pebp(1),twbp(1),qwp(1),rwp(1),rp(1),
     *                  zp(1),zcurvp(1)
            read(200,*) pebp(number),twbp(number),qwp(number),
     *                rwp(number),rp(number),zp(number),zcurvp(number)
          close(200)
        endif

        write(iunit1+200,220) (s(i),peb(i),twb(i),qw(i),rvwald(i),
     *                        rmi(i),z(i),zcurv(i), i=1,number)
        write(iunit1+200,230) pebp(1),pebp(number)
        write(iunit1+200,230) twbp(1),twbp(number)
        write(iunit1+200,230) qwp(1),qwp(number)
        write(iunit1+200,230) rwp(1),rwp(number)
        write(iunit1+200,230) rp(1),rp(number)
        write(iunit1+200,230) zp(1),zp(number)
        write(iunit1+200,230) zcurvp(1),zcurvp(number)

        !read(iunit1,300) a
        a = '******'
        write(iunit1+200,300) a
        !read(iunit1,400) a, itmp
        a = 'ISTART    ='
        write(iunit1+200,400) a, 1
        !read(iunit1,400) a, itmp
        a = 'NCFE      ='
        write(iunit1+200,400) a, 0
        !read(iunit1,400) a, itmp
        a = 'IPOSCF    ='
        write(iunit1+200,400) a, 2
        !read(iunit1,400) a, itmp
        a = 'NUPLUS    ='
        write(iunit1+200,400) a, 0
        !read(iunit1,400) a, itmp
...
c=======================================
      open(iunit5,file='table.dat',status='unknown')

      do 30 i=1,number
        !write(iunit5,210) s(i),peb(i),pebp(i),qw(i),qwp(i),rvwald(i),
        write(iunit5,*) s(i),peb(i),pebp(i),qw(i),qwp(i),rvwald(i),
     *   rwp(i),rmi(i),rp(i),twb(i),twbp(i),
     *   z(i),zp(i),zcurv(i),zcurvp(i)
 30   continue
      endfile iunit5
      close(iunit5)
      go to 80
