...
c=======================================
        if(ishort.ne.0) then
          if(m.eq.mstart) then
            open(iunit1+600,file='Int_eddybl.dat',
     *             status='unknown')
            write(iunit1+600,'(2a)') 'variables=x,Retheta,cfe,delta,
     *dstar,theta,tauw,utau,ztau,Retau,'
            open(iunit1+300,file='Stat_eddybl.dat',status='unknown')
      write(iunit1+300,'(2a)') 'variables=x,z,zod,zplus,uave,
     *vave,wave,pave,tave,mut,kinetic'
          endif
          call wall(m)
          call StatSave(m)
c=======================================
...
c=======================================================================
 ...
 120  format(1p2e12.4)
 130  format(//2x,'Disk files and PROFIL2C.DAT '
...
c=======================================================================
      ...
      character a*12,newfil*7,pform*9, fname*400
      common/close/ xc(nxc)
...
c----------------------------------
 ...
 10   continue
      io=iu(1)
      open(io,file='Check_eddybl.prt',form=pform,status=newfil)
      write(io,200)
      read(iin,100) a,irestart
      write(io,100) a,irestart
      read(iin,'(a)') fname
      write(io,'(a)') fname
      read(iin,100) a,iu(8)
      write(io,100) a,iu(8) 
...
c       Open disk files
c----------------------------------
      open(iu(2),file='table.dat',status='old')
      open(iu(3),file='Profile_outflow.dat',status=newfil)
      !open(iu(4),file='input.dat',status='old')
      if(irestart.eq.0) then
        open(iu(4),file='Profile_inflow.dat',status='old')
      else
        open(iu(4),file=trim(fname),status='old')
      endif
...
c=======================================================================
      ...
      subroutine StatSave(m)
c=======================================================================
c    S A V E STATISTIC   D A T A   F O R   P L O T T I N G
c=======================================================================
      include 'comeb.'
      character znum*4
c=======================================================================
      ...
      arcit=asin(bnew/denom)
      uplus(1)=0.
      write(iunit1+300,'("Zone T=Stat_",I04.4,",I=",I4)')
     *         m, iedge
      do 10 n=1,iedge
        if(n.ne.1) uplus(n)=ue*(asin((2.*asqrd*fp(n)-bnew)/denom)+arcit)
     *                     /(aaa*uplusb)
...
c=======================================
        ...
        rmutmp = viscon*(temp*te*tref)**vispow/(su+temp*te*tref)
        !print *, 'viscon = ', viscon, 'vispow = ', vispow
        restar = r1*re
        pestar = pe*r1*u1*u1
        tstar  = temp*te*tref
        uestar=u1*ue
        ...
        vtil = restar*uestar*amustr/(sqrt(2.d0*x))*(vp(n)-fp(n))
     *         *R*tstar/pestar
        write(iunit1+300,180) s,yodelt*xnden*eps, yodelt,yplus(n),
     *          fp(n)*ue*u1, 0,wdim,pe*r1*u1*u1,
     *          temp*te*tref,ep(n)*rmutmp,edim
 10   continue
      call dumpit(m)
      call dimsav(m)
      return
c=======================================================================
 180  format(1p11e19.9)
c=======================================================================
      end
      subroutine propm(n)
c=======================================================================
...
c=======================================
      ...
      do 10 n=1,iedge
        !read(iunit4,210) dy(n),etn(n),eto(n),fn(n),fo(n),tn(n),to(n),
        read(iunit4,*) dy(n),etn(n),eto(n),fn(n),fo(n),tn(n),to(n),
     *               vn(n),vo(n),wtn(n),wto(n),raton(n),ratoo(n),
     *               wwn(n),wwo(n),uun(n),uuo(n),uvn(n),uvo(n),
     *               vvn(n),vvo(n),deluv(n)
 10   continue
      !read(iunit4,220) dhelp,done,dtwo,d1help,eten,eteo,itstop,
...
c=======================================
      do 100 i=1,number
!        read(iunit2,230) as(i),ape(i),apep(i),aqw(i),aqwp(i),
...
c=======================================================================
 ...
 210  format(1x,1p22e15.6)
 220  format(1x,1p6e15.6,i4,1p11e15.6)
 230  format(1x,1p15e15.6)
c=======================================================================
      end
      subroutine update
c=======================================================================
...
c=======================================================================
      ...
      write(iunit1+600,200) s,rethet,cfe,yestar,disp,theta,taud,
     * upstar,eps*xn(2)/yplus(2),yestar/(eps*xn(2)/yplus(2))
     ...
     *                  eps*xn(2)/yplus(2),yestar/(eps*xn(2)/yplus(2))
      return
c=======================================================================
 200  format(1p10e15.6)
 100  format(
