...
!! InitMultiBlock:
!!     Initiate maximum possible idim, jdim, kdim, nb required for
!!     reading/writing multi-block grid & solutions (imax,jmax,kmax,nbmax)
!!  WritePlot3DGridSingleWhole:
!!     Write Plot3D single-block whole format grid file
!!  WritePlot3DGridMultiWhole:
!!     Write Plot3D multi-block whole format grid file
!!  WritePlot3DSolSingleWhole:
!!     Write Plot3D single-block whole format solution file
!!  WritePlot3DSolMultiWhole:
!!     Write Plot3D multi-block whole format solution file
!! NASA Plot3d Single-block and multi-block file
!! https://www.grc.nasa.gov/www/wind/valid/plot3d.html
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module MFileIO
  implicit none
  integer, private :: imax, jmax, kmax, nbmax
!  parameter( imax = 4000, jmax = 2000, kmax =  800, nbmax = 10)
  logical :: IsMultiBlockInit = .true.
  contains
     !Read Plot3D plane format function file with dimensions specified
     !fn (input): file name
    ...
    !Write Plot3D single-block whole format grid file
     ...
     !x,y,z(input): 3 dimensional arrays containing grid
     !               dimensions are idimxjdimxkdim
     subroutine WritePlot3DGridSingleWhole(fn,idim,jdim,kdim,x,y,z)
       character(*), intent(in) :: fn
       integer, intent(in) :: idim, jdim, kdim
       real(8), intent(in), dimension(idim,jdim,kdim) :: x, y, z
       integer :: i, j, k
       ...
       write(11)(((x(i,j,k),i=1,idim),j=1,jdim),k=1,kdim)&
              ...
              , (((z(i,j,k),i=1,idim),j=1,jdim),k=1,kdim)
       close(11)
     end subroutine WritePlot3DGridSingleWhole
     ...
     !jdim (input): size of the 2nd dimension
     !kdim (input): size of the 3rd dimension
     !vars(input): 4 dimensional arrays storing the data
     !               dimensions are idim*jdim*kdim*5
     ! mach:  freestream Mach number
     ! alpha: freestream angle-of-attack
     ! reyn:  freestream Reynolds number
     ! time:  time
    subroutine WritePlot3DSolSingleWhole(fn,idim,jdim,kdim,vars, &
                                         mach, alpha, reyn, time)
       character(*), intent(in) :: fn
       integer, intent(in) :: idim, jdim, kdim
       real(8), intent(in), dimension(idim,jdim,kdim,5) :: vars
       real(8), intent(in) :: mach, alpha, reyn, time
       integer :: i, j, k, n
       
       
       open(unit=11,file=fn,form='unformatted',status='unknown')
       write(11)idim,jdim,kdim
       write(11)mach, alpha, reyn, time      
       write(11)((((vars(i,j,k,n),i=1,idim),j=1,jdim),k=1,kdim),n=1,5)
       close(11)
    end subroutine WritePlot3DSolSingleWhole



     subroutine InitMultiBlock(i_max, j_max, k_max, nb_max)
        integer, intent(in) :: i_max, j_max, k_max, nb_max
       ...
       IsMultiBlockInit = .true.
        
     
     end subroutine InitMultiBlock
     ...
     !jdim (input): size of the 2nd dimension
     !kdim (input): size of the 3rd dimension
     !  nb (input): number of blocks
     !x,y,z(input): 3 dimensional arrays containing grid
     !               dimensions are imax*jmax*kmax*nbmax
     ! imax, jmax, kmax, nbmax are global varialbes that determine
     ! maximum possible grid sizes and number of blocks
     subroutine WritePlot3DGridMultiWhole(fn,idim,jdim,kdim,nb,x,y,z)
       character(*), intent(in) :: fn
       integer, intent(in) :: nb, idim(nbmax), jdim(nbmax), kdim(nbmax)
       real(8), intent(in), dimension(imax,jmax,kmax,nbmax) :: x, y, z
       integer :: i, j, k, m
       
       
       if(.not.IsMultiBlockInit) then
           print *, 'Need to call InitMultiBlock first before using this subroutine'
           stop
       endif
       
       open(unit=11,file=fn,form='unformatted',status='unknown')
       write(11) nb
       write(11) (idim(m), jdim(m), kdim(m), m = 1, nb)
       
       do m = 1, nb          
          write(11)(((x(i,j,k,m),i=1,idim(m)),j=1,jdim(m)),k=1,kdim(m))&
                 , (((y(i,j,k,m),i=1,idim(m)),j=1,jdim(m)),k=1,kdim(m))&
                 , (((z(i,j,k,m),i=1,idim(m)),j=1,jdim(m)),k=1,kdim(m))
       enddo
       close(11)       
     end subroutine WritePlot3DGridMultiWhole

     subroutine WritePlot3DSolMultiWhole(fn,idim,jdim,kdim,nb,vars, &
                                         mach, alpha, reyn, time)
       character(*), intent(in) :: fn
       integer, intent(in) :: nb, idim(nbmax), jdim(nbmax), kdim(nbmax)
       real(8), intent(in), dimension(imax,jmax,kmax,nbmax,5) :: vars
       real(8), intent(in) :: mach, alpha, reyn, time
       integer :: i, j, k, m, n
       ...
       do m = 1, nb
          write(11)mach, alpha, reyn, time      
          write(11) &
          ((((vars(i,j,k,m,n),i=1,idim(m)),j=1,jdim(m)),k=1,kdim(m)),n=1,5)
       enddo    
       close(11)
    end subroutine WritePlot3DSolMultiWhole
