...
!! ------------------------------------------ !!
subroutine examine_mesh_2d(u, v, nx,ny, is_good, skewness)
real(kind=8), dimension(:,:), intent(in) :: u, v
integer, intent(in) :: nx, ny
integer, intent(out) :: is_good
real(kind=8), intent(out) :: skewness
real(kind=8), dimension(2,nx-1,ny-1) :: ddi, ddj
real(kind=8), dimension(nx-1,ny-1) :: jacobian, costheta 
real(kind=8) :: pio2

pio2 = 3.141592653d0 / 2.d0

ddi(1,:,:) = u(2:nx,1:ny-1) - u(1:nx-1,1:ny-1)
ddi(2,:,:) = v(2:nx,1:ny-1) - v(1:nx-1,1:ny-1)
ddj(1,:,:) = u(1:nx-1,2:ny) - u(1:nx-1,1:ny-1)
ddj(2,:,:) = v(1:nx-1,2:ny) - v(1:nx-1,1:ny-1)

jacobian = ddi(1,:,:)*ddj(2,:,:) - ddi(2,:,:)*ddj(1,:,:)

! ---check mesh elligibility--- !
is_good = 0
if(jacobian(1,1)<0.d0) then
    if(all(jacobian<0.d0)) then
        is_good = 1
    endif
elseif(jacobian(1,1)>0.d0) then
    if(all(jacobian>0.d0)) then
        is_good = 1
    endif
endif
...
! ---check skewness--- !
costheta = (ddi(1,:,:)*ddj(1,:,:) + ddi(2,:,:)*ddj(2,:,:)) / sqrt(sum(ddi**2,1)*sum(ddj**2,1))
skewness = maxval(abs(dacos(costheta) - pio2) / pio2)
end subroutine
