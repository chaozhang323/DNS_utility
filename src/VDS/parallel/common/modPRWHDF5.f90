...
!      CALL h5gcreate_f(file_id,trim(hslab%gname), group_id, hdferr)
      CALL h5screate_simple_f(hslab%rank, hslab%dimsm, memspace, hdferr)
      CALL h5screate_simple_f(hslab%rank, hslab%dimsf, dspace_id, hdferr)
!         CALL h5dcreate_f(group_id, trim(hslab%dname(nv)), &
!                          H5T_NATIVE_DOUBLE, dspace_id, dset_id, hdferr)
...
!       print *, 'offset(3) = ', offset(3)
    ...
    ! parallel version
     ...
     subroutine ReadHDF5sol_P(fname,gname,kdim,idim,jdim,kst,ist,jst,vars,time)
       character(*), intent(in) :: fname, gname
       integer, intent(in) :: kdim, idim, jdim
       integer, intent(in) :: kst, ist, jst
       real(8), intent(out) :: vars(:,:,:,:)
...
! -----------------------------------------------------------------------
    ...
    ! Read 2D data
    subroutine ReadHDF5_2D_1V_serial(hslab, buffer,current_num)
      type(tp_rdwt_hdf5), intent(in) :: hslab
    ...
    end subroutine ReadHDF5_2D_1V_serial

    subroutine WriteHDF5_2D_1V_serial(hslab, buffer,current_num)
      type(tp_rdwt_hdf5), intent(in) :: hslab
    ...
    end subroutine WriteHDF5_2D_1V_serial
