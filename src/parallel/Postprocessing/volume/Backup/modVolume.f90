...
!    subroutine CalGradient_tsjplane(idx,x,y,z,didx,djdx,dkdx,didy,djdy,dkdy,didz,djdz,dkdz,vin,dvindj,grad_v,ist_tsjplane,kst_tsjplane)
 ...
 !   print *, 'dvindj = ', dvindj(1,1)
    ...
    subroutine CalGradient_tsiplane(idx,jmax,kmax,buffer_grid,vin,dvindi,grad_v,jst_tsiplane,kst_tsiplane)
       type(index_output), intent(in) :: idx
       real(8), intent(in) :: buffer_grid(:,:,:,:)
       real(8), intent(in) :: vin(:,:)                                 !!!!!!!
 ...
 !   print *, 'dvindj = ', dvindj(1,1)
       ...
       do k=1, klen
         do j=1, jlen
           do i=1, ilen
               iin = 1
               jin = idx%jst+(j-1)*idx%jsp-jst_tsiplane+1
               kin = idx%kst+(k-1)*idx%ksp-kst_tsiplane+1
               dvdx= ddgrid_tsiplane(iin,jin,kin,jmax,kmax,buffer_grid(jin,kin,1,1:12),vin,dvindi(jin,kin),1)
               dvdy= ddgrid_tsiplane(iin,jin,kin,jmax,kmax,buffer_grid(jin,kin,1,1:12),vin,dvindi(jin,kin),2)
               dvdz= ddgrid_tsiplane(iin,jin,kin,jmax,kmax,buffer_grid(jin,kin,1,1:12),vin,dvindi(jin,kin),3)
!               dvdx= ddgrid_tsiplane(iin,jin,kin,jmax,kmax,buffer_grid(kin,jin,1,1:12),vin,dvindi(jin,kin),1)
!               dvdy= ddgrid_tsiplane(iin,jin,kin,jmax,kmax,buffer_grid(kin,jin,1,1:12),vin,dvindi(jin,kin),2)
!               dvdz= ddgrid_tsiplane(iin,jin,kin,jmax,kmax,buffer_grid(kin,jin,1,1:12),vin,dvindi(jin,kin),3)
               grad_v(i,j,k) = sqrt(dvdx**2+dvdy**2+dvdz**2)
           enddo
         enddo
       enddo
    end subroutine CalGradient_tsiplane
